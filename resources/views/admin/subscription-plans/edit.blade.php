...
@section('title', 'Admin - Edit Subscription Plan')
        ...
        <form action="{{ route('admin.subscription-plans.update', $plan->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $plan->name) }}"
                        class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-red-500 focus:border-red-500" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="user_type" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-red-500 focus:border-red-500" required>
                        <option value="customer" {{ old('user_type', $plan->user_type) === 'customer' ? 'selected' : '' }}>Customer</option>
                        <option value="chef" {{ old('user_type', $plan->user_type) === 'chef' ? 'selected' : '' }}>Chef</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-red-500 focus:border-red-500">{{ old('description', $plan->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="yearly_price"
            ...
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">
                    Features
                    <span class="text-gray-400 font-normal text-xs">(JSON array)</span>
                </label>
                <textarea name="features" rows="4"
                    class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-red-500 focus:border-red-500 font-mono text-sm">{{ old('features', json_encode($plan->features ?? [])) }}</textarea>
            ...
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">
                    Limits
                    <span class="text-gray-400 font-normal text-xs">(JSON object)</span>
                </label>
                <textarea name="limits" rows="3"
                    class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-red-500 focus:border-red-500 font-mono text-sm">{{ old('limits', json_encode($plan->limits ?? [])) }}</textarea>
            ...
            @if($plan->subscriptions()->count() > 0)
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-700">
                    ...
                    This plan has <strong>{{ $plan->subscriptions()->count() }}</strong> active subscription(s). Deactivate instead of deleting.
                </div>
            @endif
